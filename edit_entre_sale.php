...
$sale = find_by_id('entre_sales', (int)$_GET['id']);
...
if (isset($_POST['update_sale'])) {
    ...
    if (empty($errors)) {
        ...
        $sql = "UPDATE entre_sales SET";
        $sql .= " product_id= '{$p_id}',qty={$s_qty},price='{$s_total}',date='{$s_date}', custnr='{$custnr}', comment='{$comment}', mac='{$s_mac}'";
        $sql .= " WHERE id ='{$sale['id']}'";
        $result = $db->query($sql);

        if ($result && $db->affected_rows() === 1) {
            if ($s_qty != $product['m_storage']) {
                update_entre_qty($qty_change, $p_id);
                $session->msg('s', "Sale updated.");
                redirect('edit_entre_sale.php?id=' . $sale['id'], false);
            } else {
                $session->msg('s', "Sale updated.");
                redirect('edit_entre_sale.php?id=' . $sale['id'], false);
            }
            ...
            redirect('entre_sales.php', false);
        }
    } else {
        $session->msg("d", $errors);
        redirect('edit_entre_sale.php?id=' . (int)$sale['id'], false);
    }
}
        ...
        <div class="panel">
            <div class="panel-heading clearfix">
                ...
                <div class="pull-right">
                    <a href="entre_sales.php" class="btn btn-primary">Show all sales</a>
                </div>
            </div>
            <div class="panel-body">
                <table class="table table-bordered">
                    ...
                    <tr>
                        <form method="post" action="edit_entre_sale.php?id=<?php echo (int)$sale['id']; ?>">
                            <td id="s_name">
